...
const Dashboard = () => {
  ...
  // Expect studentData as a flat student object (not nested)
  const studentId = location.state?.studentId || location.state?.studentData?.student_id;
  const initialStudentData = location.state?.studentData;
  ...
  useEffect(() => {
    // Check if we already have student data from the previous page (flat object)
    if (initialStudentData) {
      console.log("Using existing student data from location state");
      console.log("Full student object:", initialStudentData);
      console.log("Available fields:", Object.keys(initialStudentData));
